...
class PlayEquipment(models.Model):
    ...
    def get_active_renovation_work_order(self):
        if hasattr(self, "active_renovation_work_order"):
            return self.active_renovation_work_order
        from inspections.work_orders import WorkOrder

        return (
            WorkOrder.objects
            .filter(equipment=self, order_type=WorkOrder.TYPE_RENOVATION)
            .exclude(status__in=[WorkOrder.STATUS_DONE, WorkOrder.STATUS_CANCELLED])
            .order_by("renovation_year", "due_date", "planned_date", "created_at")
            .first()
        )

    @property
    def has_pending_renovation(self):
        return self.get_active_renovation_work_order() is not None
