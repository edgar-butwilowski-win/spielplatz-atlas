...
from inspections.planning import cancel_task_and_create_follow_up_task, rebuild_planning_for_organization, refresh_task_statuses
from tenants.models import Organization

from .permissions import get_active_profile_for_organization, require_inspection_permission, require_internal_view_permission, require_org_admin_permission
from .views import create_default_answers

ACTIVE_TASK_STATUSES = [InspectionTask.STATUS_OPEN, InspectionTask.STATUS_PLANNED, InspectionTask.STATUS_OVERDUE, InspectionTask.STATUS_SUSPENDED]
CLOSED_TASK_STATUSES = [InspectionTask.STATUS_COMPLETED, InspectionTask.STATUS_CANCELLED]
OVERDUE_RELEVANT_TASK_STATUSES = [InspectionTask.STATUS_OPEN, InspectionTask.STATUS_PLANNED, InspectionTask.STATUS_OVERDUE]
...
class InspectionTaskPlanningForm(forms.ModelForm):
    assigned_to = PlanningUserChoiceField(queryset=get_user_model().objects.none(), required=False, widget=forms.Select(attrs={"class": "form-select"}))
    ...
    def get_assignable_users(self):
        ...
        return get_user_model().objects.filter(is_active=True).filter(
            ...
            | (
                ...
                & Q(profile__role__in=[UserProfile.ROLE_ORG_ADMIN, UserProfile.ROLE_INSPECTOR])
            )
...
class CancelInspectionTaskForm(forms.Form):
    cancellation_reason = forms.CharField(label="Abbruchgrund", min_length=3, widget=forms.Textarea(attrs={"class": "form-control form-control-sm", "rows": 2, "placeholder": "Warum kann diese Kontrolle nicht durchgeführt werden?"}))


def get_planning_scope(user):
    if user.is_superuser:
        return {"organization": None, "is_superadmin": True, "can_manage": True, "can_inspect": True, "can_view_internal": True}
    ...
    if not profile or not profile.may_view_internal:
        return None
    return {"organization": profile.organization, "is_superadmin": False, "can_manage": profile.may_manage_organization, "can_inspect": profile.may_inspect, "can_view_internal": profile.may_view_internal}
...
def get_inspection_task_for_user(task_id, user):
    task = get_object_or_404(InspectionTask.objects.select_related("organization", "playground", "assigned_to"), id=task_id)
...
def user_can_start_inspection_task(task, user):
    return bool(user and (user.is_superuser or not task.assigned_to_id or task.assigned_to_id == user.id))
...
def task_status_count(queryset, status_key, today):
    if status_key == InspectionTask.STATUS_OVERDUE:
        return queryset.filter(status__in=OVERDUE_RELEVANT_TASK_STATUSES, due_date__lt=today).count()
    if status_key in [InspectionTask.STATUS_OPEN, InspectionTask.STATUS_PLANNED]:
        return queryset.filter(status=status_key, due_date__gte=today).count()
    return queryset.filter(status=status_key).count()


def task_status_count_map(queryset, choices):
    today = timezone.localdate()
    return [{"key": key, "label": label, "count": task_status_count(queryset, key, today)} for key, label in choices]


def add_planning_forms(tasks, user=None):
    task_list = list(tasks[:100])
    for task in task_list:
        task.planning_form = InspectionTaskPlanningForm(instance=task, organization=task.organization)
...
def inspection_planning(request):
    ...
    filtered_tasks = tasks.filter(status__in=ACTIVE_TASK_STATUSES) if status_filter == "active" else tasks
    ...
    return render(request, "internal/inspection_planning.html", {**scope, "selected_organization": selected_organization, "organizations": organizations, "tasks": add_planning_forms(filtered_tasks, request.user), "status_filter": status_filter, "inspection_type_filter": inspection_type_filter, "status_choices": [("active", "Aktive Aufträge")] + list(InspectionTask.STATUS_CHOICES), "inspection_type_choices": Inspection.TYPE_CHOICES, "task_counts_by_status": task_status_count_map(tasks, InspectionTask.STATUS_CHOICES), "can_manage_selected_organization": scope["can_manage"] and selected_organization is not None, "today": timezone.localdate()})
...
def my_inspections(request):
    ...
    tasks = InspectionTask.objects.select_related("organization", "playground", "assigned_to").filter(status__in=ACTIVE_TASK_STATUSES)
    if request.user.is_superuser:
        assigned_tasks = tasks.filter(assigned_to=request.user)
        unassigned_tasks = tasks.filter(assigned_to__isnull=True)
    else:
        assigned_tasks = tasks.filter(organization=scope["organization"], assigned_to=request.user)
        unassigned_tasks = tasks.filter(organization=scope["organization"], assigned_to__isnull=True)
    refresh_task_statuses(assigned_tasks)
    refresh_task_statuses(unassigned_tasks)
    today = timezone.localdate()
    upcoming_limit = today + timedelta(days=30)
    return render(request, "internal/my_inspections.html", {**scope, "today": today, "assigned_today": assigned_tasks.filter(planned_date=today), "assigned_overdue": assigned_tasks.filter(status=InspectionTask.STATUS_OVERDUE), "assigned_upcoming": assigned_tasks.exclude(status=InspectionTask.STATUS_OVERDUE).filter(Q(planned_date__isnull=True, due_date__lte=upcoming_limit) | Q(planned_date__gte=today, planned_date__lte=upcoming_limit)), "unassigned_due": unassigned_tasks.filter(status__in=[InspectionTask.STATUS_OPEN, InspectionTask.STATUS_OVERDUE])[:25]})
...
def rebuild_inspection_planning(request):
    ...
    messages.success(request, f"Die Kontrollplanung wurde neu berechnet. Erstellt: {result['created']}, aktualisiert: {result['updated']}, unverändert: {result['unchanged']}.")
...
def update_inspection_task(request, task_id):
    task = get_object_or_404(InspectionTask.objects.select_related("organization", "playground"), id=task_id)
    ...
    form = InspectionTaskPlanningForm(request.POST, instance=task, organization=task.organization)
    if form.is_valid():
        task = form.save(commit=False)
        if task.status not in CLOSED_TASK_STATUSES:
            task.status = InspectionTask.STATUS_PLANNED if task.planned_date or task.assigned_to_id else InspectionTask.STATUS_OPEN
        try:
            task.full_clean()
            task.save()
    ...
    if not form.is_valid():
        for errors in form.errors.values():
            for error in errors:
                messages.error(request, error)
        return redirect(planning_redirect_url(task.organization_id))
    follow_up_task = cancel_task_and_create_follow_up_task(task, form.cleaned_data["cancellation_reason"])
    if follow_up_task:
        messages.success(request, f"Der Kontrollauftrag wurde abgebrochen und ein Folgeauftrag wurde angelegt. Nächste Fälligkeit: {follow_up_task.due_date:%d.%m.%Y}.")
    else:
        messages.success(request, "Der Kontrollauftrag wurde abgebrochen. Es konnte kein Folgeauftrag erzeugt werden, weil keine aktive Kontrollregel vorhanden ist.")
    return redirect(planning_redirect_url(task.organization_id))
...
def start_inspection_from_task(request, task_id):
    ...
    if task.playground.is_inspection_suspended:
        messages.error(request, "Für diesen Spielplatz ist die Inspektion aktuell ausgesetzt. Es kann keine neue Kontrolle erfasst werden.")
        return redirect("internal:my_inspections")
    inspection = Inspection.objects.create(playground=task.playground, inspection_type=task.inspection_type, inspected_at=timezone.localdate(), inspector=request.user, result=Inspection.RESULT_OK)
