...
def get_status_bucket_choices():
    return [
        ("active", "Aktive Kontrollaufträge"),
        ("overdue", "Überfällig"),
...
def apply_status_bucket_filter(tasks, bucket, today, due_soon_until):
    ...
    if bucket == "due_soon":
        return tasks.filter(
            status__in=[InspectionTask.STATUS_OPEN, InspectionTask.STATUS_PLANNED],
            due_date__gte=today,
            due_date__lte=due_soon_until,
        )
...
def build_control_status_metrics(tasks, today):
    ...
    return {
        ...
        "completed_on_time_recent_count": completed_tasks.filter(
            completed_by_inspection__inspected_at__gte=recent_completion_cutoff,
            completed_by_inspection__inspected_at__lte=F("due_date"),
        ).count(),
...
def control_status(request):
    ...
    if not scope:
        raise PermissionDenied("Keine Berechtigung für den Kontrollstatus.")
    ...
    return render(
        request,
        "internal/control_status.html",
        {
            **scope,
            "selected_organization": selected_organization,
            "organizations": organizations,
            "metrics": build_control_status_metrics(tasks, today),
            "tasks": build_task_rows(filtered_tasks, today),
            "today": today,
            ...
            "status_bucket_choices": get_status_bucket_choices(),
